import { PolicyEvaluationCompletedEventHandler } from './policy-evaluation-completed.event-handler';
...
describe('PolicyEvaluationCompletedEventHandler', () => {
  let handler: PolicyEvaluationCompletedEventHandler;
  ...
  beforeEach(() => {
    ...
    handler = new PolicyEvaluationCompletedEventHandler(prisma, commandBus);
  });

  const event = new PolicyEvaluationCompletedEvent(
    'dec-1', 'corr-1', 'org-1', 'ws-1', 2, ['policy-1', 'policy-2'], new Date()
  );
  ...
  it('should upsert state with expectedPolicyCount and dispatch command', async () => {
    ...
    expect(mockUpsert).toHaveBeenCalledWith({
      ...
      create: expect.objectContaining({ correlationId: 'corr-1', expectedPolicyCount: 2 }),
      update: { expectedPolicyCount: 2 },
    });
