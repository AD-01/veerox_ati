...
export class LicenseService {
  ...
  async issueLicense(props: {
    ...
    if (tx) return this.issueLicenseInTransaction(props, tx);
    return await this.prisma.$transaction(async (transaction) => this.issueLicenseInTransaction(props, transaction));
  }

  async issueAndActivateLicense(props: {
    organizationId: string;
    workspaceId: string;
    productId: string;
    subscriptionId: string;
    ...
    if (tx) {
      const licenseId = await this.issueLicenseInTransaction(props, tx);
      await this.activateLicenseInTransaction(licenseId, props.organizationId, props.workspaceId, tx);
      return licenseId;
    }
    return this.prisma.$transaction(async (transaction) => {
      const licenseId = await this.issueLicenseInTransaction(props, transaction);
      await this.activateLicenseInTransaction(licenseId, props.organizationId, props.workspaceId, transaction);
      return licenseId;
    });
  }

  private async issueLicenseInTransaction(props: {
    organizationId: string;
    workspaceId: string;
    productId: string;
    subscriptionId?: string;
      ...
      let license = await this.licenseRepo.findByWorkspaceAndProduct(props.workspaceId, props.productId, tx);
      ...
      try {
        ...
        if (error.code === 'P2002') {
          ...
          const existing = await this.licenseRepo.findByWorkspaceAndProduct(props.workspaceId, props.productId, tx);
  ...
  async activateLicense(licenseId: string, organizationId: string, workspaceId: string): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      await this.activateLicenseInTransaction(licenseId, organizationId, workspaceId, tx);
    });
  }

  private async activateLicenseInTransaction(licenseId: string, organizationId: string, workspaceId: string, tx: any): Promise<void> {
      const license = await this.licenseRepo.findById(licenseId, tx);
